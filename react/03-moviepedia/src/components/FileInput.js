...
function FileInput({ name, onChange, value, initialPreview }) {
  ...
  useEffect(() => {
    ...
    // useEffect에서는 사이드 이펙트(외부에 무언가 변경된다.)를 정리하는 기능을 제공한다.
    // 리턴을 해줄 때 정리하는 함수를 리턴해주면 사이트 이펙트를 정리할 수 있다.
    const nextPreview = URL.createObjectURL(value); // createObjectURL 의 파라미터는 Blod, file, MediaSourceObject, null 타입의 데이터가 들어와야한다.
    setPreview(nextPreview);

    // 디펜더시 리스트([value]) 값이 바뀌면 새로 콜백을 실행하는데 이 전에 리액트는
